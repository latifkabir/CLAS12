...
//        TableLoader.Fill(); //Not needed
	 ...
	 */ 
        ...
        //----------> Disable (by setting to -1.0) the following line for the time being to remove error flag<---------------------        
        double calcTime = -1.0; //tableLoader.calc_Time(doca, dMax, tMax, alphaDeg, bField, iSec, iSL);
        ...
        for (int i = 0; i < 100; i++) {
            ...
            //----------> Disable the following line (by setting to -1.0) for the time being to remove error flag<---------------------
