...
import static org.jlab.dc_calibration.init.Histograms.h2timeVtrkDocaVZ;
import static org.jlab.dc_calibration.init.Histograms.hArrWire;
import static org.jlab.dc_calibration.init.Histograms.htrkDoca;
...
public class TestFile implements ActionListener, Runnable
{
			...
			for (int j = 0; j < nSL; j++)
			{
				for (int k = 0; k < nThBinsVz; k++)
				{
					...
					mapOfFitLines.get(new Coordinate(i, j, k))
							.setParameters(mapOfUserFitParameters.get(new Coordinate(i, j, k)));
				}
			}
		}
	...
	protected void processData()
	{
		...
		{// && icounter < 100
			...
			{// tbTracks.getNTrks() > 0 &&
				// System.out.println(tbCrs.getNCrs() + " " + tbTracks.getNTrks() + " " +
				// tbHits.getNrows() + " " + tbSegments.getNrows());
		...
		for (int i = 0; i < nSectors; i++)
		{
			for (int j = 0; j < nSL; j++)
			{
				for (int k = 0; k < nThBinsVz; k++)
				{
					...
					// trkDocasvsTime.get(new Coordinate(i, j)).draw(mapOfFitLines.get(new
					// Coordinate(i, j, k)), "same");

					trkDocasvsTimeProfiles.get(new Coordinate(i, j)).cd(k);
					trkDocasvsTimeProfiles.get(new Coordinate(i, j))
							.draw(h2timeVtrkDocaVZ.get(new Coordinate(i, j, k)).getProfileX());
					// trkDocasvsTimeProfiles.get(new Coordinate(i, j)).draw(mapOfFitLines.get(new
					// Coordinate(i, j, k)), "same");
				}
	...
	public void runFitter()
	{
		...
		double tMaxSL[] =
		{ 155.0, 165.0, 300.0, 320.0, 525.0, 550.0 };
		// Now start minimization
		double parSteps[] =
		{ 0.00001, 0.001, 0.01, 0.01, 0.0001 };
		double pLow[] =
		{ prevFitPars[0] * 0.4, prevFitPars[1] * 0.0, prevFitPars[2] * 0.4, prevFitPars[3] * 0.4,
				prevFitPars[4] * 0.0 };
		double pHigh[] =
		{ prevFitPars[0] * 1.6, prevFitPars[1] * 5.0, prevFitPars[2] * 1.6, prevFitPars[3] * 1.6,
				prevFitPars[4] * 1.6 };
			...
			for (int j = 0; j < nSL; j++)
			{
				for (int k = 0; k < nThBinsVz; k++)
				{
					mapOfFitFunctions.put(new Coordinate(i, j, k),
							new DCFitFunction(h2timeVtrkDocaVZ.get(new Coordinate(i, j, k)).getProfileX(), j, k,
									isLinearFit));
					mapOfFitParameters.put(new Coordinate(i, j, k), new MnUserParameters());
					for (int p = 0; p < nFreePars; p++)
					{
						mapOfFitParameters.get(new Coordinate(i, j, k)).add(parName[p], prevFitPars[p], parSteps[p],
								pLow[p], pHigh[p]);
					}
					mapOfFitParameters.get(new Coordinate(i, j, k)).setValue(2, tMaxSL[j]);// tMax
																							// for
																							// SLth
																							// superlayer
					mapOfFitParameters.get(new Coordinate(i, j, k)).fix(2);
					MnMigrad migrad = new MnMigrad(mapOfFitFunctions.get(new Coordinate(i, j, k)),
							mapOfFitParameters.get(new Coordinate(i, j, k)));
					...
					if (!min.isValid())
					{
						...
						MnMigrad migrad2 = new MnMigrad(mapOfFitFunctions.get(new Coordinate(i, j, k)), min.userState(),
								new MnStrategy(2));
