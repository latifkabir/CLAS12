...
public class DCFitFunction implements FCNBase
{
	...
	// Added boolean dummy to avoid "Method has the same erasure add(Set) as above
	...
	public double getChisqUsingXProfiles(int discardThBins, double par[])
	{
		...
		for (int th = 0; th < nThBinsVz; th++)
		{
			...
			for (int i = 0; i < profileX.getDataSize(0); i++)
			{
				...
				sliceX = h2tvXnorm.sliceX(i);// Histogram of y-values for all data falling in the
												// ith x-slice or x-bin.
				// nSliceX = (int) sliceX.getEntries();//derived histos always have 0 entries, so
				// let's use integral()
				...
				if (meanErrorType == 0)
				{
					measTimeErr = profileX.getDataEY(i); // 4/5/17: Replaced this with the followign
															// two lines
				}
				else if (meanErrorType == 1)
				{
					measTimeErr = profileX.getDataEY(i) / Math.sqrt(nSliceX); // Using Error =
																				// RMS/sqrt(N)
				}
