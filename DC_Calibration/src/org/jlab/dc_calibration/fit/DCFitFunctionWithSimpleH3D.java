...
public class DCFitFunctionWithSimpleH3D implements FCNBase
{
	...
	public DCFitFunctionWithSimpleH3D(Map<Coordinate, SimpleH3D> h3BvTvX, int iSector, int iSuperlayer,
			boolean isLinear)
	{
	...
	public double valueOf(double[] par)
	{
		...
		for (int th = 0 + discardThBins; th < nThBinsVz2 - discardThBins; th++)
		{
			...
			if (th == (nThBinsVz2 / 2) - 1 || th == (nThBinsVz2 / 2) + 1)
			{
				continue;
			}
			...
			for (int i = 0; i < nBinsX; i++)
			{
				for (int j = 0; j < nBinsY; j++)
				{
					for (int k = 0; k < nBinsZ; k++)
					{
						...
						if (statErr == statErr && statErr > 0.0 && docaNorm < 0.8)
						{ 
